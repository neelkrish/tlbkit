...
def hit_miss_comparison(m1, m2, c1, c2, name):
    log.log('%s misses for miss/no-miss set: %.2f vs %.2f. Cycles per iteration: %.1f vs %.1f',
            name, m1, m2, c1, c2)
    if m2 >= 1:
        raise Exception(
            'The no-miss rate is 1 or larger, this likely means the assumptions (eviction set) of this experiment are wrong.')
    if m2 >= 0.3:
        raise Exception(
            'miss rate is quite large for the no-miss set! Possibly wrong set or noisy data.')
    if m1 < 1:
        raise Exception(
            'miss rate is quite small for the no-miss set! Possibly wrong set or noisy data.')
    if m1 <= m2:
        raise Exception(
            'More misses in the miss vs no-miss case makes no sense. Wrong eviction set or noisy data.')
    if c1 <= c2:
        raise Exception(
            'More cycles in the miss vs no-miss case makes no sense. Wrong eviction set or noisy data.')
    extra_misses = m1-m2
    extra_cycles = c1-c2
    log.log('Extra misses: %.2f extra cycles: %.2f cycles per miss: %.2f',
            extra_misses, extra_cycles, extra_cycles/extra_misses)
...
def test_latency_and_miss_per_iteration(pagelist, event, name):
    ...
    for iterations in range(0, 20000000, 5000000):
        ...
        cmdlist = ["perf", "stat", "-x,", "-e", event+",cycles",
                   "./c/tvlb-verbatim", "-r", str(iterations), "-c"+str(cpu)]+pagelist_args
        sys.stdout.flush()
        p1 = subprocess.Popen(
            cmdlist, stderr=subprocess.STDOUT, stdout=subprocess.PIPE)
    ...
    log.log('done. misses: %.1f, cycles: %.1f',
            event_per_iteration, cycles_per_iteration)
...
if __name__ == "__main__":
    ...
    for l1_size in range(1, 10):
        ...
        l1misses1, l1cycles1 = test_latency_and_miss_per_iteration(
            l1_pagelist_misses, 'dtlb_load_misses.stlb_hit', 'l1 miss test size ' + str(l1_size))
        if l1misses1 > 1:
            log.log('Found')
            log.log('')
            break
    for l2_size in range(1, 16):
        ...
        l2misses1, l2cycles1 = test_latency_and_miss_per_iteration(
            l2_pagelist_misses, 'dtlb_load_misses.miss_causes_a_walk', 'l2 miss test size ' + str(l2_size))
        if l2misses1 > 1:
            log.log('Found')
            log.log('')
            break
    ...
    l1misses1, l1cycles1 = test_latency_and_miss_per_iteration(
        l1_pagelist_misses, 'dtlb_load_misses.stlb_hit', 'l1 miss list')
    l1misses2, l1cycles2 = test_latency_and_miss_per_iteration(
        l1_pagelist_no_misses, 'dtlb_load_misses.stlb_hit', 'l1 no-miss list')
    l2misses1, l2cycles1 = test_latency_and_miss_per_iteration(
        l2_pagelist_misses, 'dtlb_load_misses.miss_causes_a_walk', 'l2 miss list')
    l2misses2, l2cycles2 = test_latency_and_miss_per_iteration(
        l2_pagelist_no_misses, 'dtlb_load_misses.miss_causes_a_walk', 'l2 no-miss list')
