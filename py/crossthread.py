        ...
        for set_id1 in range(nsets):
            for set_id2 in range(nsets):
                if level == 1:
                    ...
                    pagelist1 = tlblib.generate_set_l2_general(
                        set_id1, setsize)
                    pagelist2 = tlblib.generate_set_l2_general(
                        set_id2, setsize)
                ...
                cmdlist = ["perf", "stat", "-x,", "-e",
                           ",".join(events), "./c/tvlb-verbatim", "-r", str(iterate)]
                print('popen 1:', cmdlist+pagelist_args1, 'cpu', c1)
                print('popen 2:', cmdlist+pagelist_args2, 'cpu', c2)
                p1 = subprocess.Popen(
                    cmdlist+["-c"+str(c1)]+pagelist_args1, stderr=subprocess.STDOUT, stdout=subprocess.PIPE)
                p2 = subprocess.Popen(
                    cmdlist+["-c"+str(c2)]+pagelist_args2, stderr=subprocess.STDOUT, stdout=subprocess.PIPE)
                ...
                print('yset1:', yset1[event],
                      yset2[event], c1, c2, set_id1, set_id2)
                misses[set_id2, set_id1] = (
                    (float(yset1[event])+yset2[event])/iterate)/2.0
            # print misses
...
if __name__ == "__main__":
    ...
    numpy.set_printoptions(threshold=numpy.inf,
                           linewidth=numpy.nan, precision=1)
    l2size = 128

#    test_cores('-D',3,l2size,'dtlb_load_misses.miss_causes_a_walk', cpu_uarch + '-stlb-sets.pdf', 2)
    l1dtlb = test_cores('-D', 3, 16, 'dtlb_load_misses.stlb_hit',
                        cpu_uarch + '-dtlb-sets.pdf', 1)
    l1itlb = test_cores('-C', 3, 16, 'itlb_misses.stlb_hit',
                        cpu_uarch + '-itlb-sets.pdf', 1)
    ...
    plt.savefig('results-crossthread/' + cpu_uarch +
                '-tlbsets-ht.pdf', bbox_inches='tight')
    plt.savefig('results-crossthread/' + cpu_uarch +
                '-tlbsets-ht.png', bbox_inches='tight')
    plt.close()
